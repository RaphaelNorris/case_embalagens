...
# -----------------
# Converters/mapeamentos para FACAS
# -----------------
...
def _map_row_to_oracle_facas(row: Dict[str, Any]) -> Dict[str, Any]:
    """
    Mapeamento das colunas do SQL Server para Oracle para a tabela FACAS.
    """
    return {
        "CODFACA": row.get("CodFaca"),
        "TIPO": row.get("Tipo"),
        "DESCRTIPOFACA": row.get("DescrTipoFaca"),
        "FORNECEDOR": row.get("Fornecedor"),
        "MODULO": row.get("Modulo"),
        "ARQUIVO": row.get("Arquivo"),
        "LARGURA": row.get("Largura"),
        "COMPRIMENTO": row.get("Comprimento"),
        "PECASPORFACA": row.get("PecasPorFaca"),
        "LOCALIZACAO": row.get("Localizacao"),
        "FIGURA": row.get("Figura"),
        "STATUS": row.get("Status"),
        "PRECO": row.get("Preco"),
        "COMPLAMINA": row.get("CompLamina"),
...
# -----------------
# Tarefa principal para FACAS
# -----------------
def etl_facas(**context):
    # 1) Extrai do SQL Server
    sql = "SELECT * FROM FACAS"
    ...
    try:
        ...
        log.info("SQL Server -> %s registros extraídos da tabela FACAS.", len(rows))
    ...
    for r in rows:
        try:
            mapped_row = _map_row_to_oracle_facas(r)
    ...
    merge_sql = """
        ...
        USING (
            SELECT
                :CODFACA CODFACA,
                :TIPO TIPO,
                :DESCRTIPOFACA DESCRTIPOFACA,
                :FORNECEDOR FORNECEDOR,
                :MODULO MODULO,
                :ARQUIVO ARQUIVO,
                :LARGURA LARGURA,
                :COMPRIMENTO COMPRIMENTO,
                :PECASPORFACA PECASPORFACA,
                :LOCALIZACAO LOCALIZACAO,
                :FIGURA FIGURA,
                :STATUS STATUS,
                :PRECO PRECO,
                :COMPLAMINA COMPLAMINA,
                :IDCLIENTE IDCLIENTE,
                :OBS OBS,
                :CODIGOERP CODIGOERP,
                :CHAVEESTADOFERRAMENTA CHAVEESTADOFERRAMENTA,
                :DESATIVADOSN DESATIVADOSN,
                :REFUGOCLIENTE REFUGOCLIENTE,
                :DATACRIACAOREGISTRO DATACRIACAOREGISTRO
            FROM dual
        ) S
        ON (T.CODFACA = S.CODFACA)
        WHEN MATCHED THEN UPDATE SET
            T.TIPO = S.TIPO,
            T.DESCRTIPOFACA = S.DESCRTIPOFACA,
            T.FORNECEDOR = S.FORNECEDOR,
            T.MODULO = S.MODULO,
            T.ARQUIVO = S.ARQUIVO,
            T.LARGURA = S.LARGURA,
            T.COMPRIMENTO = S.COMPRIMENTO,
            T.PECASPORFACA = S.PECASPORFACA,
            T.LOCALIZACAO = S.LOCALIZACAO,
            T.FIGURA = S.FIGURA,
            T.STATUS = S.STATUS,
            T.PRECO = S.PRECO,
            T.COMPLAMINA = S.COMPLAMINA,
            ...
            T.REFUGOCLIENTE = S.REFUGOCLIENTE,
            T.DATACRIACAOREGISTRO = S.DATACRIACAOREGISTRO
        WHEN NOT MATCHED THEN INSERT (
            CODFACA, TIPO, DESCRTIPOFACA, FORNECEDOR, MODULO, ARQUIVO,
            LARGURA, COMPRIMENTO, PECASPORFACA, LOCALIZACAO, FIGURA, STATUS,
            PRECO, COMPLAMINA, IDCLIENTE, OBS, CODIGOERP, CHAVEESTADOFERRAMENTA,
            DESATIVADOSN, REFUGOCLIENTE, DATACRIACAOREGISTRO
        ) VALUES (
            S.CODFACA, S.TIPO, S.DESCRTIPOFACA, S.FORNECEDOR, S.MODULO, S.ARQUIVO,
            S.LARGURA, S.COMPRIMENTO, S.PECASPORFACA, S.LOCALIZACAO, S.FIGURA, S.STATUS,
            S.PRECO, S.COMPLAMINA, S.IDCLIENTE, S.OBS, S.CODIGOERP, S.CHAVEESTADOFERRAMENTA,
            S.DESATIVADOSN, S.REFUGOCLIENTE, S.DATACRIACAOREGISTRO
        )
    """
    ...
    try:
        ...
        dst.commit()
        log.info("Oracle <- %s registros mergeados com sucesso na tabela FACAS.", len(payload))
...
with DAG(
    ...
    schedule=None,
    catchup=False,
    tags=["etl", "sqlserver", "oracle", "facas"],
    default_args={
        'retries': 2,
    }
):
