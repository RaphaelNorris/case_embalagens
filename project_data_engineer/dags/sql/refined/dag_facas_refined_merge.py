...
def _map_row_to_refined_facas(row: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if not hasattr(_map_row_to_refined_facas, 'logged_structure'):
        log.info("Estrutura da linha recebida da TRUSTED: %s", list(row.keys())[:10])
        _map_row_to_refined_facas.logged_structure = True
    
    return {
        "CD_FACA": _convert_to_varchar(row.get("CODFACA"), "-1", "CD_FACA"),
        "ST_STATUS": _convert_to_varchar(row.get("STATUS"), " ", "ST_STATUS"),
        "VL_COMPLAMINA": _convert_to_number(row.get("COMPLAMINA"), 0),
        "FL_DESATIVADOSN": _convert_to_varchar(_handle_nan(row.get("DESATIVADOSN"), 0), "0", "FL_DESATIVADOSN"),
    }

# -----------------
# Tarefa principal
# -----------------
def etl_facas_trusted_to_refined(**context):
    ...
    while True:
        ...
        try:
            ...
            log.info("TRUSTED -> Página %s: %s registros extraídos da tabela FACAS.", 
            ...
            for r in rows:
                try:
                    mapped_row = _map_row_to_refined_facas(r)
            ...
            if payload:
                ...
                _process_batch_facas_refined_merge(payload, batch_count + 1)
...
def _process_batch_facas_refined_merge(payload, batch_number):
    """Processa um lote de registros no Oracle REFINED com MERGE"""
    merge_sql = """
        MERGE INTO FACAS dest
        USING (
            SELECT 
                :CD_FACA as CD_FACA,
                :ST_STATUS as ST_STATUS,
                :VL_COMPLAMINA as VL_COMPLAMINA,
                :FL_DESATIVADOSN as FL_DESATIVADOSN
            FROM DUAL
        ) src
        ON (dest.CD_FACA = src.CD_FACA)
        WHEN MATCHED THEN
            UPDATE SET 
                ST_STATUS = src.ST_STATUS,
                VL_COMPLAMINA = src.VL_COMPLAMINA,
                FL_DESATIVADOSN = src.FL_DESATIVADOSN
        WHEN NOT MATCHED THEN
            INSERT (
                CD_FACA, ST_STATUS, VL_COMPLAMINA, FL_DESATIVADOSN
            ) VALUES (
                src.CD_FACA, src.ST_STATUS, src.VL_COMPLAMINA, src.FL_DESATIVADOSN
            )
    """
...
# -----------------
with DAG(
    dag_id="etl_trusted_to_refined_facas_merge",
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    tags=["etl", "trusted", "refined", "facas"],
    default_args={
        'retries': 2,
    }
):
    trusted_to_refined = PythonOperator(
        task_id="trusted_to_refined_facas",
        python_callable=etl_facas_trusted_to_refined,
    )