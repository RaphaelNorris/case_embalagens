...
def etl_maquina_trusted_to_refined(**context):
    ...
    try:
        ...
        if payload:
            ...
            # 3) Processa todos os registros com MERGE
            _process_batch_maquina_refined_merge(payload)
            total_processed = len(payload)
            log.info("Processamento concluído: %s registros válidos processados.", total_processed)
        else:
...
def _process_batch_maquina_refined_merge(payload):
    """Processa todos os registros no Oracle REFINED com MERGE"""
    merge_sql = """
        MERGE INTO MAQUINA dest
        USING (
            SELECT 
                :TX_MAQUINA as TX_MAQUINA,
                :TX_TIPO as TX_TIPO,
                :QT_NRDECORES as QT_NRDECORES,
                :ID_GRUPOMAQUINA as ID_GRUPOMAQUINA
            FROM DUAL
        ) src
        ON (dest.TX_MAQUINA = src.TX_MAQUINA)
        WHEN MATCHED THEN
            UPDATE SET 
                TX_TIPO = src.TX_TIPO,
                QT_NRDECORES = src.QT_NRDECORES,
                ID_GRUPOMAQUINA = src.ID_GRUPOMAQUINA
        WHEN NOT MATCHED THEN
            INSERT (
                TX_MAQUINA, TX_TIPO, QT_NRDECORES, ID_GRUPOMAQUINA
            ) VALUES (
                src.TX_MAQUINA, src.TX_TIPO, src.QT_NRDECORES, src.ID_GRUPOMAQUINA
            )
    """
    ...
    try:
        ...
        for record in payload:
            try:
                ...
                log.error("Erro ao processar registro no MERGE: %s", str(e))
                error_count += 1
                # Continua processando os demais registros
        
        dst.commit()
        log.info("REFINED <- %s registros processados com sucesso, %s erros.", success_count, error_count)
            
    except Exception as e:
        log.error("Erro ao executar MERGE no Oracle REFINED: %s", str(e))
        dst.rollback()
        raise
    finally:
        dst.close()
...
# -----------------
with DAG(
    dag_id="etl_trusted_to_refined_maquina_merge",
