...
def etl_paradas_trusted_to_refined(**context):
    ...
    log.info("REFINED -> Executando TRUNCATE na tabela PARADAS...")
    dst_conn = _make_oracle_conn_refined()
    try:
        with dst_conn.cursor() as cur:
            cur.execute("TRUNCATE TABLE PARADAS")
        dst_conn.commit()
        log.info("REFINED -> TRUNCATE executado com sucesso na tabela PARADAS.")
    except Exception as e:
    ...
    while True:
        ...
        try:
            ...
            if payload:
                ...
                _process_batch_paradas_refined(payload, batch_count + 1)
...
def _process_batch_paradas_refined(payload, batch_number):
...
# -----------------
with DAG(
    dag_id="etl_trusted_to_refined_paradas",
