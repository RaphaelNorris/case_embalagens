...
def _map_row_to_refined_paradas(row: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if not hasattr(_map_row_to_refined_paradas, 'logged_structure'):
        log.info("Estrutura da linha recebida da TRUSTED: %s", list(row.keys())[:10])
        _map_row_to_refined_paradas.logged_structure = True
    
    return {
        "ID_PARADA": _convert_to_varchar(row.get("ID_PARADA") or row.get("PARADA"), "-1", "ID_PARADA"),
        "TX_DESCRICAO": _convert_to_varchar(row.get("TX_DESCRICAO") or row.get("DESCRICAO"), " ", "TX_DESCRICAO"),
        "FL_USADACONVERSAO": _convert_to_varchar(_handle_nan(row.get("FL_USADACONVERSAO") or row.get("USADACONVERSAO"), 0), "0", "FL_USADACONVERSAO"),
        "FL_DESATIVADA": _convert_to_varchar(_handle_nan(row.get("FL_DESATIVADA") or row.get("DESATIVADA"), 0), "0", "FL_DESATIVADA"),
        "FL_EXTERNA": _convert_to_varchar(_handle_nan(row.get("FL_EXTERNA") or row.get("EXTERNA"), 0), "0", "FL_EXTERNA"),
    }

# -----------------
# Tarefa principal
# -----------------
def etl_paradas_trusted_to_refined(**context):
    ...
    while True:
        ...
        try:
            ...
            log.info("TRUSTED -> Página %s: %s registros extraídos da tabela PARADAS.", 
            ...
            for r in rows:
                try:
                    mapped_row = _map_row_to_refined_paradas(r)
            ...
            if payload:
                ...
                _process_batch_paradas_refined_merge(payload, batch_count + 1)
...
def _process_batch_paradas_refined_merge(payload, batch_number):
    """Processa um lote de registros no Oracle REFINED com MERGE"""
    merge_sql = """
        MERGE INTO PARADAS dest
        USING (
            SELECT 
                :ID_PARADA as ID_PARADA,
                :TX_DESCRICAO as TX_DESCRICAO,
                :FL_USADACONVERSAO as FL_USADACONVERSAO,
                :FL_DESATIVADA as FL_DESATIVADA,
                :FL_EXTERNA as FL_EXTERNA
            FROM DUAL
        ) src
        ON (dest.ID_PARADA = src.ID_PARADA)
        WHEN MATCHED THEN
            UPDATE SET 
                TX_DESCRICAO = src.TX_DESCRICAO,
                FL_USADACONVERSAO = src.FL_USADACONVERSAO,
                FL_DESATIVADA = src.FL_DESATIVADA,
                FL_EXTERNA = src.FL_EXTERNA
        WHEN NOT MATCHED THEN
            INSERT (
                ID_PARADA, TX_DESCRICAO, FL_USADACONVERSAO, FL_DESATIVADA, FL_EXTERNA
            ) VALUES (
                src.ID_PARADA, src.TX_DESCRICAO, src.FL_USADACONVERSAO, src.FL_DESATIVADA, src.FL_EXTERNA
            )
    """
...
# -----------------
with DAG(
    dag_id="etl_trusted_to_refined_paradas_merge",
    start_date=datetime(2025, 1, 1),
    schedule=None,
    catchup=False,
    tags=["etl", "trusted", "refined", "paradas"],
    default_args={
        'retries': 2,
    }
):
    trusted_to_refined = PythonOperator(
        task_id="trusted_to_refined_paradas",
        python_callable=etl_paradas_trusted_to_refined,
    )