...
def etl_pedidos_raw_to_trusted(**context):
    ...
    print(f"[PEDIDOS] Iniciando ETL para tabela {TARGET_TABLE}")

    # 1) TRUNCATE na tabela de destino antes da carga - IGUAL À DAG FUNCIONAL
    print("Oracle -> Executando TRUNCATE na tabela PEDIDOS...")
    dst_conn = _make_oracle_conn(ORACLE_CONN_ID_TRUSTED)
    try:
        with dst_conn.cursor() as cur:
            cur.execute("TRUNCATE TABLE PEDIDOS")
        dst_conn.commit()
        print("Oracle -> TRUNCATE executado com sucesso na tabela PEDIDOS.")
    except Exception as e:
        print(f"Erro ao executar TRUNCATE no Oracle: {str(e)}")
        dst_conn.rollback()
        raise
    finally:
        dst_conn.close()
...
# -----------------
with DAG(
    dag_id="etl_pedidos_raw_to_trusted",
